...
flowchart TD
    ...
    orig -->|"아니오"| local["로컬 정적<br/>/products/&lt;상품id&gt;.webp"]
    local --> use
...
파생본 → 원본 → 로컬 정적(`/products/<상품id>.webp`)으로 두 단계 내려앉는 것이 리졸버의 강등 사슬이다.
파일서버가 멈춰 있거나 파생본 생성이 아직 안 끝난 상태에서도 화면이 깨지지 않게 하는 것이 목적이고,
로컬 정적 이미지는 인프라 연동 이전부터 레포에 있던 그 파일들이다.

로컬 정본은 `webp_optimized` 파생본과 같은 **800px WebP**로 맞춰 뒀다 — 강등돼도 화면 품질이 눈에 띄게
흔들리지 않게 하려는 것이다. 원래는 1024px PNG 50장이었는데, 그대로 두면 쓰이지도 않는 무거운 파일이
매 배포마다 정적 자산으로 함께 올라간다.
...
sequenceDiagram
    ...
    P->>P: 확장자 · content-type · 5MiB 검사<br/>파일명을 서버가 재조립
    P->>DB: 상품 존재 · 삭제 여부 확인
...
| 용량 | 서비스 계층 5 MiB(경계값 포함) — 공용 인프라 file-server의 업로드 상한과 판정 기준까지 동일하다. 컨테이너 한도는 그보다 조금 넓게(6 MB) 두어 오류 문구를 우리가 낸다 |
