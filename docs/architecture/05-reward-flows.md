...
상승 엣지에 `goal_rewarded_at IS NULL` 래치를 더한 세 조건이 모두 맞을 때만 한 번 나간다. 목표 걸음수와 보상
...
`GET /api/v1/steps/leaderboard`는 비즈니스 시간대(`app.steps.zone-id`) 기준 **오늘** 걸음 상위 10과
요청자 본인의 순위를 함께 돌려준다. 권한은 다른 steps 엔드포인트와 같은 `CUSTOMER` 전용이다.

```json
{ "date": "2026-08-21",
  "entries": [ { "rank": 1, "name": "소소한하루", "steps": 14791 } ],
  "me": { "rank": 17, "steps": 8000 } }
```

- 노출은 표시명과 걸음 수뿐이다 — 이메일·userId는 쿼리에서 선택하지 않는다.
- 걸음 0은 순위에서 제외하고, 정렬은 `(steps desc, userId asc)` 하나라 동점에서도 목록과 `me.rank`가
  어긋나지 않는다. 오늘 기록이 없으면 `me.rank`는 null이다.
- **시드 참가자는 조회 시점에 lazy 보강**된다 — 리뷰 작성자 계정들의 그날 걸음 행이 없으면 그 자리에서
  채운다. 스케줄러가 아니라 조회 시점에 채우는 이유는 데모 서버가 상시 가동이 아니어서다 — 자정에
  꺼져 있어도 첫 조회가 그날 몫을 채운다.
- 보강 값은 (이메일 · 날짜)를 시드로 3,000~15,000보를 **결정적으로** 만든다. 하루 안에서는 재조회해도
  같고, 날짜가 바뀌면 달라진다. **이미 있는 행은 절대 덮어쓰지 않으며** 보상 경로(포인트·알림)도 타지
  않는다 — 실사용자 걸음은 기존 `POST /api/v1/steps` 그대로 섞인다.
- 스키마 변경 없음(`user_daily_steps` 재사용). 랭킹 화면은 30초 주기로 갱신하고, 걸음 제출이 캐시를
  무효화하므로 제출 직후에는 즉시 반영된다.
...
건너뛴다 — 이 경우 알림도 나가지 않는다. 반면 쿠폰 정의 자체가 사용 불가 상태면 건너뛰지 않고
**가입 전체가 400 `INVALID_COUPON`으로 실패**한다. 쿠폰 지갑에 들어오는 경로는 `SIGNUP`(가입)·`CODE_REDEEM`(코드 입력)·
